...
internal var globalNetwork: FFNN = {
    if let url = Bundle.ocrBundle().url(forResource: "OCR-Network", withExtension: nil) {
...
open class IDCardOCR {
    ...
    fileprivate var xMergeRadius = CGFloat(1)
    ///Radius in y axis for merging blobs
    fileprivate var yMergeRadius = CGFloat(3)
    ///Confidence must be bigger than the threshold
    fileprivate var confidenceThreshold:Float = 0.1
                ...
                DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async(group: group) {
                    ...
                    blobs.forEach {
                        ...
                        do {
                            ...
                            if networkResults.max() > self.confidenceThreshold {
                                
                                for (idx, _) in networkResults.enumerated().sorted(by: { $0.0.element > $0.1.element}) {
                                    
                                    let character = indexToCharacter(idx)
                                    
                                    if character == Character("X") && numberRecognizedString.lengthOfBytes(using: String.Encoding.utf8) != 17 {
                                        continue
                                    }
    ...
    func preprocessImage(_ image: UIImage) -> UIImage {
        ...
        func getDodgeBlendImage(_ inputImage: UIImage) -> UIImage {
            ...
            var processedImage = dodgeBlendFilter.imageFromCurrentFramebuffer(with: UIImageOrientation.up)
        ...
        var processedImage = thresholdFilter.imageFromCurrentFramebuffer(with: UIImageOrientation.up)
    ...
    internal func extractBlobs(_ image: UIImage) -> [(UIImage, CGRect)] {
        ...
        for label in minMaxXYLabelDict.keys {
            ...
            if minMaxCorrect && notToTall && notToWide && notToShort && notToThin && notToSmall && positionIsOK &&
                ...
                let temp = data[minY...maxY].map({return Array($0[(minX + 2)...(maxX - 2)])})
                let temp1 = Array(temp);
                let transposedData = temp1.transpose();
...
//                let transposedData = Array(data[minY...maxY].map({return $0[(minX + 2)...(maxX - 2)]})).transpose() // [y][x] -> [x][y]
        ...
        outputImages.sort { return $0.0.1.origin.x < $0.1.1.origin.x } // 从左到右排列
     ...
     */
    internal func convertImageToFloatArray(_ image: UIImage, resize: Bool = true) -> [Float] {
