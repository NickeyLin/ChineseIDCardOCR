...
class FocusView: UIView, CAAnimationDelegate {
    ...
    func startScaningAnimation() {
        ...
        laser.backgroundColor = UIColor.clear.cgColor
        ...
        laser.add(animation, forKey: "scaningAnimation")
    }
    ...
    fileprivate func laserImage(_ width: CGFloat, height: CGFloat) -> UIImage {
    ...
    fileprivate func gridImage(_ width: CGFloat, height: CGFloat) -> UIImage {
        ...
        // 先画竖线
        stride(from: 0, to: Int(width), by: 3).map { CGFloat($0) } .forEach {
            context?.beginPath()
        ...
        // 再画横线
        stride(from: 0, to: Int(height), by: 3).map { CGFloat($0) }.forEach {
            context?.beginPath()
            context?.move(to: CGPoint(x: 0, y: $0))
